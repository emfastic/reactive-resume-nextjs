...
export default function SkillsInterestsResumeSection({ skillObject, interestObject }: any) {
    ...
    const isSkillsOrInterests = Object.keys(skillObject).length > 0 || Object.keys(interestObject).length > 0

    if (Object.keys(skillObject).length > 0 && Object.keys(interestObject).length > 0) {
  ...
  return (
    <Box>
                  {isSkillsOrInterests ? (<><Heading size="sm" textTransform="uppercase" mb='2'>
                    {headingTitle}
                  </Heading>
                  <Divider /></>) : (<></>)}
