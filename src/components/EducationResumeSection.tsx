...
function EducationResumeSection({ educationObject }: any) {
  ...
  const educationItems = educationList.map(
    (educationObject: EducationExperience) => {
      return (
        <Box key={educationObject.key} mb="2">
          <Flex justify="space-between" fontWeight={"semibold"}>
            <Box>{educationObject.school}</Box>
            <Box>{educationObject.location}</Box>
          </Flex>
          <Flex justify="space-between">
            <Box>
              {educationObject.degreeType} in {educationObject.major}
              {educationObject.minor
                ? `, Minor in ${educationObject.minor}`
                : ""}
            </Box>
            <Box>{formatDate(educationObject.gradDate)}</Box>
          </Flex>
          <Box>
            {educationObject.gpa !== "0.00"
              ? `GPA: ${educationObject.gpa}/4.00`
              : ""}
          </Box>
        </Box>
      );
